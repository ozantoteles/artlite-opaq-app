...
def generate_channel_in_range(unique_id, min_value=1, max_value=81):
    try:
        # Hash the unique ID using SHA-256
        hash_object = hashlib.sha256(unique_id.encode())
        # Convert the hash to an integer
...
channel = generate_channel_in_range(unique_id)
print(f"Generated Channel for {unique_id}: {channel}")
...
custom_conf.set_custom_conf(addh=addrh, addl=addrl, CHAN=channel)  # Ensure this channel matches the receiver
